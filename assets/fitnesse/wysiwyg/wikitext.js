...
Wysiwyg.prototype.wikitextToFragment = function (wikitext, contentDocument) {
  ...
  function inParagraph() {
    return getSelfOrAncestor(holder, "p");
  }

  function inDefinition() {
    return $(holder).parents().andSelf().filter("p.meta, p.comment").get(0);
  }

  function inTable() {
    return getSelfOrAncestor(holder, "table");
  }

  function inEscapedTable() {
    return $(holder).parents().andSelf().filter("table.escaped").get(0);
  }

  function inHashTable() {
    return $(holder).parents().andSelf().filter("table.hashtable").get(0);
  }

  function inTableRow() {
    return getSelfOrAncestor(holder, "tr", "tbody");
  }

  function inAnchor() {
    return getSelfOrAncestor(holder, "a");
  }

  function inNestedText() {
    return getSelfOrAncestor(holder, "div");
  }

  function inEscapedText() {
    return getSelfOrAncestor(holder, "tt");
  }

  function inCodeBlock() {
    return getSelfOrAncestor(holder, "pre");
  }
  ...
  function handleLine(line) {
    ...
    while (true) {
      ...
      switch (matchNumber) {
        case 1:     // bolditalic
          if (inEscapedTable() || inEscapedText()) {
            break;
          }
        ...
        case 10:     // close escaped
          ...
          continue;
        case 11:    // Wiki link
          if (inEscapedTable() || inEscapedText() || inCodeBlock()) {
            break;
          }
        ...
        case 15:    // Hash table key-value separator ':'
          if (!inHashTable() || inEscapedTable() || inEscapedText() || inCodeBlock()) {
            break;
          }
          handleTableCell(0);
          continue;
